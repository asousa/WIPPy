 ...
 *   -Reworked input parameters
 *   -Fixed ray-segment crossing detection
 */
...
typedef struct {
  ...
  float center_lat;          // latitude of lightning source
...
FILE *readFile(char *fileName, FILE *filePtr, rayT *ray, long f, float center_lat);
 ...
 * directory - path to the folder containing rayfiles and damping files
 * i0 - peak current of lightning stroke (amps)
 * center_lat - latitude of the lightning stroke, in degrees (geomagnetic)
 * low_freq - lower frequency in Hz 
 * high_freq - higher frequency in Hz
 * L_TARG - L-shell to calculate scattering at
 */
...
int main(int argc, char *argv[])
{
  ...
  if(argc != 7) 
    { printf("Wrong number of input arguments (readOne)\n");  exit(0); }
  ...
  ptrR  = (FILE *)readFile( fileR, ptrR, &BR, upper_freq, center_lat);
  dampR = (FILE *)readDamp( dampFileR, dampR, &BR, upper_freq);

  while( 1 ) {
    ptrL = (FILE *)readFile( fileL, ptrL, &TL, lower_freq, center_lat);
    dampL = (FILE *)readDamp( dampFileL, dampL, &TL, lower_freq);
    ptrR = (FILE *)readFile( fileR, ptrR, &TR, upper_freq, center_lat);
          ...
          cellT *lat_arr[][NUM_TARGS], double *EAarrPtr)
{
  ...
  //           BR->tg[ BR->numSteps-1 ],
  //           TR->tg[ TR->numSteps-1 ] );
  // dlat = fabs(TL->lat[0] - BL->lat[0]);
  // dfreq= fabs(TL->f - TR->f);
  // sprintf("dlat: %g\n",dlat);
  // div_lat_step = 1.0/DIV_LAT_NUM;
  // div_freq_step = 1.0/DIV_FREQ_NUM;


  //printf("BLf: %d, BRf: %d, TLf: %d, TRf: %d\n",BL->f, BR->f, TL->f, TR->f);

dlat  = (TL->lat[0] - BL->lat[0]);
dfreq = (TR->f - TL->f);

// number of latitude and frequency steps in fine-scale interpolation
...
//      t_prev = t- T_STEP;
    ...
    for(div_lat=0; div_lat<=1; div_lat += 1.0/DIV_LAT_NUM) {
      ...
      //printf("Passing: t: %2.2f, l_int: %2.2f, l_int_prev: %2.2f, lat_int: %2.2f, lat_prev: %2.2f\n",t,l_int,l_int_prev,lat_int,lat_int_prev);
        ...
        double l_int_prev, double lat_int_prev, int iTarg)
{
  ...
  // printf("Getting: t: %2.2f, l_int: %2.2f, l_int_prev: %2.2f, lat_int: %2.2f, lat_prev: %2.2f\n",t,l_int,l_int_prev,lat_int,lat_int_prev);
  ...
  EA_iLow  = (int)floor( (lat_low - EALimS) / EAIncr );
  EA_iHigh = (int)ceil( (lat_high - EALimS) / EAIncr );
  ...
  // fprintf(logfile,"MAYBE: %2.8f %2.8f %2.8f %2.8f %2.8f\n",t,r1ray,r2ray,lat_int_prev,lat_int);
...
void dispLatArr(cellT *lat_arr[][NUM_TARGS],  FILE *outPtr)
{
  ...
  fprintf(outPtr, "%g %g %g %g %g %g %g %g %g %g\n", 
    ...
    next->t,
    next->f,
    ((next->pwr)/(next->num_rays)/F_STEP/T_STEP),
    ...
    double l_int, double lat)
{
  ...
  // print information to file if greater than 1e-6 of initial pwr
  avePwr = ( TL->pwr[0] + BL->pwr[0] + TR->pwr[0] + BR->pwr[0] )/4.0;
//  if( pwr > (1e-6 * avePwr) )   -- HEY AUSTIN, YOU EDITED THIS 6/11/15
  // if( pwr > (WAVE_PWR_THRESH * avePwr) )
 ...
 * center_lat: lightning source latitude
        ...
        float lat, float dlong )
{
  ...
  dist_lat = (R_E+H_IONO/2) * fabs(center_lat-lat) * D2R ;
...
int outsideL(double BL_l, double TL_l, double BR_l, double TR_l, int *iTarg)
{
  ...
  for(i=0; i<NUM_TARGS; i++) {
    lowBound = L_TARG - L_MARGIN;
    upBound = L_TARG + L_MARGIN;    
    if(  ( (BL_l<=lowBound) &&    
 ...
 * columns: 
 *    0  1   2  3 4   5      6       7     8            
 *    lam x1  x2  y1  y2  EA_a   EA_b    EA_c  EA_length  
 *
 *      
 *        
 *                
...
double *initEA_Arr(void)
{
  ...
  printf("EA array: rows: %d, columns: %d \n", EA_NUMROWS, EA_NUMCOLS);
...
double getEntry(int row, int col, double *arrPtr)
{
  // printf("col: %u, row: %u, entry: %d\n",col, row, *(arrPtr + col + row*EA_NUMCOLS));
...
FILE  *readDamp( char *fileName, FILE *filePtr, rayT *ray, long f)
{
  ...
  // Read in each ray //
  while( 1 ) { 
    if(fscanf( filePtr, "%f", &(ray->pwr[i]) ) == EOF ) { 
        ...
        int iTarg, double stixP, double stixR, double stixL)
{
  ...
  // t_grid = floor(t/(1.0*T_STEP))*T_STEP;
